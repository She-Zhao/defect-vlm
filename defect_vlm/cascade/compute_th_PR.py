...
def calculate_pr_thresholds(pred_json, gt_json, output_dir, target_thresholds, target_recalls, target_precisions):
    ...
    report_lines.append("【任务 1: 给定 Conf 阈值 -> 查询 Precision, Recall & 框数量】")
    report_lines.append(f"{'Target Conf':>12} | {'Precision':>10} | {'Recall':>10} | {'留存框(送入VLM)':>15} | {'过滤框(小于阈值)':>15}")
    
    total_preds = len(global_confs)
    
    for th in target_thresholds:
        valid_idxs = np.where(global_confs >= th)[0]
        retained_boxes = len(valid_idxs)
        filtered_boxes = total_preds - retained_boxes
        ...
        report_lines.append(f"{th:>12.4f} | {p:>10.4f} | {r:>10.4f} | {retained_boxes:>15} | {filtered_boxes:>15}")
...
if __name__ == '__main__':
    ...
    PRED_JSON = "/data/ZS/defect_dataset/9_yolo_preds/自己手写的推理脚本/val_0p001/nms_fusion_th0.json"
    OUTPUT_DIR = "/data/ZS/defect-vlm/output"
    
    # ============ 配置你需要查询的目标值 ============
    # 1. 想看的置信度阈值 (看看 YOLO 在这些常规阈值下的表现)
    TEST_THRESHOLDS = [0.001, 0.002, 0.005, 0.01, 0.02, 0.05, 0.1, 0.2, 0.25]
