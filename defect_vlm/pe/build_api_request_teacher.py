...
def generate_sft_entries(
    ...
    for idx, item in enumerate(source_data):
        try:
            ...
            label = item['label']
            prompt_text = prompt_template.replace('{prior_label}', prior_label).replace('{label}', label)
...
if __name__ == "__main__":
    ...
    config = {
        "input_json": base_dir / '3_composite_images/stripe_phase123_gt_rectification/labels/train.json',
        "output_file": base_dir / '4_api_request/teacher/train/stripe_phase123_gt_rectification.jsonl',
        "prompt_idx": 2,
    }   
