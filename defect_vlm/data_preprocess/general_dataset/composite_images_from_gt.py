...
def get_group_key(item: dict) -> str:
    ...
    # 【修改点】：兼容获取 bbox，优先用抖动框，其次用原 bbox
    bbox = item.get('yolo_jitter_bbox', item.get('bbox'))
...
def process_dataset(
    ...
    # 2. 构建输出目录结构
    # e.g., output_root/stripe_phase123/images/train
    ...
    for key, items in pbar:
        ...
        # 【修改点】：提取需要画在全局图上的框（必须是裁切局部图用的那个框）
        bbox = base_item.get('yolo_jitter_bbox', base_item.get('bbox'))
        ...
        for it in items:
            ...
            # 【修改点】：这里也兼容获取一下当前 item 的框
            current_bbox = it.get('yolo_jitter_bbox', it.get('bbox'))
            
            # 全局图：画红框
            orig_with_box = draw_bbox_on_image(orig_img, current_bbox)
            # 局部图：缩放
        ...
        # === 构建 JSON Item ===
        # 【修改点 1：彻底消灭硬编码】
        # 动态计算 output_root 相对于 dataset_root 的相对目录名 (比如 "3_composite_images_general")
        rel_output_dir = os.path.relpath(output_root, dataset_root) 
        
        rel_global_path = os.path.join(rel_output_dir, project_name, "images", split, global_filename)
        rel_local_path = os.path.join(rel_output_dir, project_name, "images", split, local_filename)
        ...
        # 动态追加我们在前几步生成的珍贵测试数据（如果有的话）
        if 'jitter_iou' in base_item:
            out_item['jitter_iou'] = base_item['jitter_iou']
        if 'overlap_iou' in base_item:
            out_item['overlap_iou'] = base_item['overlap_iou']
        if 'neg_type' in base_item:
            out_item['neg_type'] = base_item['neg_type']
...
if __name__ == "__main__":
    ...
    # 2. 输出保存的根目录
    # 最终会生成到 OUTPUT_ROOT/stripe_phase123/...
    OUTPUT_ROOT = os.path.join(DATASET_ROOT, "3_composite_images_general")
    
    
    # 3. 输入参数   唯一需要修改的地方
    # 示例1：处理 Positive Train
    input_json = "/data/ZS/defect_dataset/2_paint_bbox_general/sp123_gt_positive/labels/val.json"
    ...
    input_json = "/data/ZS/defect_dataset/2_paint_bbox_general/sp123_gt_rectification/labels/val.json"
    ...
    # 示例3：处理 Negative Val
    input_json = "/data/ZS/defect_dataset/2_paint_bbox_general/sp123_gt_negative/labels/val.json"
