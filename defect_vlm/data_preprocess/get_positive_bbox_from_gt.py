...
def get_region_proposal(image: np.ndarray, bbox: list, bbox_format='coco', context_ratio: float = 1.0) -> np.ndarray:
    """
    根据bbox截取图像局部
    """
    ...
    for image_id, anns in tqdm(img_id_to_anns.items(), desc=f"Processing {split}"):
        ...
        for light in light_sources:
            ...
            for i, ann in enumerate(anns):
                ...
                # 裁剪
                crop = get_region_proposal(raw_image, bbox) # 注意这里用了0.4
