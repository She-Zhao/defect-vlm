...
def rename_ids(input_file, output_file, prefix: str='sp012'):
    # 用于统计各类样本的数量
    counts = {"pos": 0, "neg": 0, "rect": 0, "unknown": 0, "right_id":0}
    ...
    with open(input_file, 'r', encoding='utf-8') as f_in, \
        ...
        for line_num, line in enumerate(f_in, 1):
            ...
            try:
                ...
                if not any(key in str(original_id) for key in ['sp012', 'sp123']):
                    # 安全获取第一张图片的路径
                    ...
                    # 拼接新的 ID
                    new_id = f"{prefix}_gt_{type_str}_{original_id}"
                    data["id"] = new_id
                else:
                    counts["right_id"] += 1
    ...
    print(f"已经正确 (right) 样本数: {counts['right_id']}")
    if counts['unknown'] > 0:
